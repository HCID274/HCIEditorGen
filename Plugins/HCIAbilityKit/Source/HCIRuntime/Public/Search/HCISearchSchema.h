 ...
 * 检索文档：用于承载可检索的语义字段喵。
 */
struct HCIRUNTIME_API FHCIAbilitySearchDocument
{
	FString AssetPath;
	FString Id;
	FString DisplayName;
 ...
 */
struct HCIRUNTIME_API FHCIAbilitySearchIndex
{
 ...
 * 语义规范器：把解析数据映射为检索文档喵。
 */
class HCIRUNTIME_API FHCIAbilitySearchSchema
{
public:
	static FHCIAbilitySearchDocument BuildDocument(
		const FHCIParsedData& ParsedData,
