...
- 切片编号：`Step1-Slice4`
- 当前状态：`已完成（Pass）`
- 切片目标：Reimport 失败时提供可视化反馈（通知），减少仅依赖日志的排障成本。
- 验收动作（UE）：
  - 打开工程并编译 Editor；
  - 对非法 `.hciabilitykit` 触发 Reimport 失败，确认看到失败通知（含原因）；
  - 修复文件后再次 Reimport，确认成功并可继续工作。
- 通过后动作：进入 Step2-Slice1（解析逻辑从 Factory 抽离到 Runtime 服务层）。
