...
#include "AgentActions/ToolActions/Wired/Internal/HCINormalizeAssetNamingByMetadata_Internal.h"
...
namespace
{
class FHCINormalizeAssetNamingByMetadataToolAction final : public IHCIAgentToolAction
{
public:
...
private:
		...
		FString& OutResolvedAssetName)
	{
		...
		const FString TrimmedDir = HCIAssetPathUtils::TrimTrailingSlash(DestinationDir);
		...
		auto BuildPaths = [&TrimmedDir](const FString& AssetName, FString& OutAssetPath, FString& OutObjectPath)
		{
			...
			OutObjectPath = HCIAssetPathUtils::ToObjectPath(OutAssetPath, AssetName);
		};
	...
	{
		const FHCIAssetRoutingRules RoutingRules = HCI_LoadAssetRoutingRules();
		...
		Notify.Start(bIsDryRun ? TEXT("HCI: 预览资产规范化/归档中...") : TEXT("HCI: 执行资产规范化/归档中..."));
		...
		for (const FString& RawPath : AssetPaths)
		{
			...
			FHCIToolActionAssetPathNormalizer::NormalizeAssetPathVariants(RawPath, Item.SourceAssetPath, Item.SourceObjectPath);

			if (!HCIAssetPathUtils::TrySplitObjectPath(Item.SourceObjectPath, Item.SourcePackagePath, Item.SourceAssetName))
			{
			...
			Item.Prefix = HCIAssetNamingRules::DeriveClassPrefixFromAssetData(Item.AssetData);
			...
			FString GroupName = HCIAssetNamingRules::SanitizeIdentifier(GroupSanitized);
			if (RoutingRules.bGroupNamePascalCase)
			{
				GroupName = HCI_ToPascalCase(GroupName);
				GroupName = HCIAssetNamingRules::SanitizeIdentifier(GroupName);
			}
		...
		for (const FHCINormalizeItem& Item : Items)
		{
			...
			if (Item.bIsShared)
			{
				AssetBaseName = HCIAssetNamingRules::SanitizeIdentifier(Item.BaseNameFromMetadata);
				if (RoutingRules.bGroupNamePascalCase)
				{
					AssetBaseName = HCI_ToPascalCase(AssetBaseName);
					AssetBaseName = HCIAssetNamingRules::SanitizeIdentifier(AssetBaseName);
				}
			...
			TargetAssetName = HCIAssetNamingRules::SanitizeIdentifier(TargetAssetName);
			...
			DestinationDir = HCIAssetPathUtils::TrimTrailingSlash(DestinationDir);
			...
			Proposal.SourceDirectory = HCIAssetPathUtils::GetDirectoryFromPackagePath(Item.SourcePackagePath);
...
TSharedPtr<IHCIAgentToolAction> HCIToolActionFactories::MakeNormalizeAssetNamingByMetadataToolAction()
{
	return MakeShared<FHCINormalizeAssetNamingByMetadataToolAction>();
}
