...
- 切片编号：`Stage F - Slice F12`
- 当前状态：`F12 已通过（用户 UE 手测 Pass）`
- 切片目标：`ExecuteTicket` 执行前投递校验 -> `SimExecuteReceipt` 模拟执行回执桥接：基于 F11 的 `ExecuteTicket` 与最新 `ConfirmRequest/ApplyRequest/Review` 预览执行 `confirm_request_id + apply_request_id + review_request_id + selection_digest + ready_to_simulate_execute + user_confirmed` 完整性校验，桥接为 `SimExecuteReceipt` 契约与 JSON；保持 `simulate_dry_run`，不触发真实资产写入。
- 通过后动作：已切换到 `Stage F - Slice F13`（待定义），继续 F 段收尾联调。
