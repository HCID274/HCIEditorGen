...
- 切片编号：`Stage F - Slice F11`
- 当前状态：`F11 已通过（用户 UE 手测 Pass）`
- 切片目标：`ConfirmRequest` 确认后校验 -> `ExecuteTicket` 执行票据桥接：基于 F10 的 `ConfirmRequest` 与最新 `ApplyRequest/Review` 预览执行 `apply_request_id + review_request_id + selection_digest + ready_to_execute + user_confirmed` 完整性校验，桥接为可投递给后续执行器的 `ExecuteTicket` 契约与 JSON；保持 `simulate_dry_run`，不触发真实资产写入。
- 通过后动作：已切换到 `Stage F - Slice F12`（待定义），继续 F 段收尾联调。
