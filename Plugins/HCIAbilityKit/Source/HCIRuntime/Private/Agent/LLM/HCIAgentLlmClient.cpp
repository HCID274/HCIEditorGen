...
DEFINE_LOG_CATEGORY_STATIC(LogHCIAgentLlmClient, Log, All);

namespace
{
struct FHCILlmRouterEntry
{
	FString Model;
	double Weight = 0.0;
	double Current = 0.0;
...
struct FHCILlmRouterState
{
	...
	TArray<FHCILlmRouterEntry> Entries;
};

static FCriticalSection GHCILlmRouterStateMutex;
static TMap<FString, FHCILlmRouterState> GHCILlmRouterStates;
...
static bool HCI_LoadRouterStateFromJsonFile(
	...
	FHCILlmRouterState& OutState,
	FString& OutError)
{
	OutState = FHCILlmRouterState();
...
static bool HCI_SelectModelFromRouterState(FHCILlmRouterState& State, FString& OutModel)
{
	OutModel.Reset();
		...
		State.Strategy.Equals(TEXT("weighted_random"), ESearchCase::IgnoreCase))
	{
		...
		for (const FHCILlmRouterEntry& Entry : State.Entries)
		{
	...
	{
		FHCILlmRouterEntry& Entry = State.Entries[Index];
	...
	FHCILlmRouterEntry& Selected = State.Entries[BestIndex];
	...
	FString& OutReason)
{
	...
	FScopeLock Lock(&GHCILlmRouterStateMutex);
	FHCILlmRouterState* State = GHCILlmRouterStates.Find(RouterConfigPath);
	if (State == nullptr || State->SourceTimestamp != Timestamp)
	{
		FHCILlmRouterState LoadedState;
		FString LoadError;
		if (!HCI_LoadRouterStateFromJsonFile(RouterConfigPath, Timestamp, LoadedState, LoadError))
		{
			...
			GHCILlmRouterStates.Remove(RouterConfigPath);
			return false;
		}

		GHCILlmRouterStates.Add(RouterConfigPath, MoveTemp(LoadedState));
		State = GHCILlmRouterStates.Find(RouterConfigPath);
	}
...
bool FHCIAgentLlmClient::LoadProviderConfigFromJsonFile(
	...
	FString& OutError)
{
	OutConfig = FHCIAgentLlmProviderConfig();
	...
	if (HCI_TrySelectModelByRouter(Root, ResolvedConfigPath, RoutedModel, RouterReason))
	{
		...
		UE_LOG(
			LogHCIAgentLlmClient,
			Display,
			TEXT("[HCI][LlmRouter] selected_model=%s reason=%s"),
			*OutConfig.Model,
			*RouterReason);
	}
	else if (!RouterReason.IsEmpty() && !RouterReason.Equals(TEXT("router_config_missing"), ESearchCase::CaseSensitive))
	{
		UE_LOG(
			LogHCIAgentLlmClient,
			Warning,
			TEXT("[HCI][LlmRouter] fallback_to_default_model=%s reason=%s"),
			*OutConfig.Model,
			*RouterReason);
	}
...
bool FHCIAgentLlmClient::BuildChatCompletionsRequestBody(
...
TSharedPtr<IHttpRequest, ESPMode::ThreadSafe> FHCIAgentLlmClient::CreateChatCompletionsHttpRequest(
	const FHCIAgentLlmProviderConfig& Config,
...
bool FHCIAgentLlmClient::TryExtractMessageContentFromResponse(
...
void FHCIAgentLlmClient::ResetRoutingStateForTesting()
{
	FScopeLock Lock(&GHCILlmRouterStateMutex);
	GHCILlmRouterStates.Reset();
}
