...
- 切片编号：`Stage F - Slice F13`
- 当前状态：`F13 已通过（用户 UE 手测 Pass）`
- 切片目标：`SimExecuteReceipt` 回执后完整性校验 -> `SimFinalReport` 最终模拟执行报告桥接：基于 F12 的 `SimExecuteReceipt` 与最新 `ExecuteTicket/ConfirmRequest/ApplyRequest/Review` 预览执行 `execute_ticket_id + confirm_request_id + apply_request_id + review_request_id + selection_digest + ready_to_simulate_execute + simulated_dispatch_accepted + user_confirmed` 完整性校验，桥接为 `SimFinalReport` 契约与 JSON；保持 `simulate_dry_run`，不触发真实资产写入。
- 通过后动作：已切换到 `Stage F - Slice F14`（待定义），继续冻结 F 段终态收敛/交付口径。
