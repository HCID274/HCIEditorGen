...
namespace
{
static const FHCIToolArgSchema* FindArg(const FHCIToolDescriptor& Tool, const TCHAR* ArgName)
{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FHCIToolRegistryWhitelistFrozenTest,
	"HCI.Editor.AgentTools.RegistryWhitelistFrozen",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FHCIToolRegistryWhitelistFrozenTest::RunTest(const FString& Parameters)
{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FHCIToolRegistryArgsSchemaFrozenTest,
	"HCI.Editor.AgentTools.ArgsSchemaFrozen",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FHCIToolRegistryArgsSchemaFrozenTest::RunTest(const FString& Parameters)
{
	FHCIToolRegistry& Registry = FHCIToolRegistry::Get();
	Registry.ResetToDefaults();

	const FHCIToolDescriptor* TextureTool = Registry.FindTool(TEXT("SetTextureMaxSize"));
	...
	const FHCIToolArgSchema* TexturePathsArg = FindArg(*TextureTool, TEXT("asset_paths"));
	TestNotNull(TEXT("SetTextureMaxSize.asset_paths should exist"), TexturePathsArg);
	if (TexturePathsArg)
	{
		TestEqual(TEXT("asset_paths type should be string[]"), static_cast<uint8>(TexturePathsArg->ValueType), static_cast<uint8>(EHCIToolArgValueType::StringArray));
	...
	const FHCIToolArgSchema* MaxSizeArg = FindArg(*TextureTool, TEXT("max_size"));
	...
	const FHCIToolDescriptor* LodTool = Registry.FindTool(TEXT("SetMeshLODGroup"));
	...
	const FHCIToolArgSchema* LodGroupArg = FindArg(*LodTool, TEXT("lod_group"));
	...
	const FHCIToolDescriptor* LevelRiskTool = Registry.FindTool(TEXT("ScanLevelMeshRisks"));
	...
	const FHCIToolArgSchema* ScopeArg = FindArg(*LevelRiskTool, TEXT("scope"));
	const FHCIToolArgSchema* ChecksArg = FindArg(*LevelRiskTool, TEXT("checks"));
	const FHCIToolArgSchema* MaxActorCountArg = FindArg(*LevelRiskTool, TEXT("max_actor_count"));
	const FHCIToolArgSchema* ActorNamesArg = FindArg(*LevelRiskTool, TEXT("actor_names"));
	...
	if (ActorNamesArg)
	{
		TestEqual(
			...
			static_cast<uint8>(EHCIToolArgValueType::StringArray));
	...
	const FHCIToolDescriptor* SearchPathTool = Registry.FindTool(TEXT("SearchPath"));
	TestNotNull(TEXT("SearchPath should exist"), SearchPathTool);
	if (SearchPathTool)
	{
		const FHCIToolArgSchema* KeywordArg = FindArg(*SearchPathTool, TEXT("keyword"));
		TestNotNull(TEXT("SearchPath.keyword should exist"), KeywordArg);
		if (KeywordArg)
		{
			TestEqual(TEXT("keyword type should be string"), static_cast<uint8>(KeywordArg->ValueType), static_cast<uint8>(EHCIToolArgValueType::String));
	...
	const FHCIToolDescriptor* ScanMeshTriangleCountTool = Registry.FindTool(TEXT("ScanMeshTriangleCount"));
	TestNotNull(TEXT("ScanMeshTriangleCount should exist"), ScanMeshTriangleCountTool);
	if (ScanMeshTriangleCountTool)
	{
		const FHCIToolArgSchema* DirectoryArg = FindArg(*ScanMeshTriangleCountTool, TEXT("directory"));
		TestNotNull(TEXT("ScanMeshTriangleCount.directory should exist"), DirectoryArg);
		if (DirectoryArg)
		{
			TestEqual(TEXT("directory type should be string"), static_cast<uint8>(DirectoryArg->ValueType), static_cast<uint8>(EHCIToolArgValueType::String));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FHCIToolRegistryDomainsAndFlagsTest,
	"HCI.Editor.AgentTools.DomainCoverageAndFlags",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FHCIToolRegistryDomainsAndFlagsTest::RunTest(const FString& Parameters)
{
	FHCIToolRegistry& Registry = FHCIToolRegistry::Get();
	Registry.ResetToDefaults();

	const TArray<FHCIToolDescriptor> Tools = Registry.GetAllTools();
	TSet<FString> Domains;
	for (const FHCIToolDescriptor& Tool : Tools)
	{
		Domains.Add(Tool.Domain);
		if (Tool.Capability == EHCIToolCapability::ReadOnly)
		{
			TestFalse(
	...
	const FHCIToolDescriptor* RenameTool = Registry.FindTool(TEXT("RenameAsset"));
	TestNotNull(TEXT("RenameAsset should exist"), RenameTool);
	if (RenameTool)
	{
		TestEqual(
			TEXT("RenameAsset capability should be write"),
			FHCIToolRegistry::CapabilityToString(RenameTool->Capability),
			FString(TEXT("write")));
		...
		const FHCIToolArgSchema* NewNameArg = FindArg(*RenameTool, TEXT("new_name"));
