...
DEFINE_LOG_CATEGORY_STATIC(LogHCIAgentPlanPreview, Log, All);

namespace
{
static TArray<FString> HCI_ExtractAssetObjectPaths(const FHCIAgentPlanStep& Step)
{
...
static bool HCI_StepHasArgumentPlaceholder(const FHCIAgentPlanStep& Step)
{
	if (!Step.Args.IsValid())
	{
		return false;
...
static FString HCI_BuildArgsPreview(const FHCIAgentPlanStep& Step)
{
	if (!Step.Args.IsValid())
	{
		return TEXT("{}");
...
static void HCI_LocateAssetsInContentBrowser(const TArray<FString>& AssetObjectPaths)
{
	...
	{
		UE_LOG(
			LogHCIAgentPlanPreview,
			Warning,
			TEXT("[HCI][AgentPlanPreview] locate=failed reason=no_resolved_assets"));
		return;
	}
	...
	UE_LOG(
		LogHCIAgentPlanPreview,
		Display,
		TEXT("[HCI][AgentPlanPreview] locate=ok resolved_assets=%d"),
		AssetsToSync.Num());
}
...
static bool HCI_Preview_IsWriteLikeRisk(const EHCIAgentPlanRiskLevel RiskLevel)
{
...
static void HCI_AddLocateTargetUnique(
	const EHCIAgentExecutionLocateTargetKind Kind,
	...
	TArray<FHCIAgentExecutionLocateTarget>& OutTargets)
{
	...
	FHCIAgentExecutionLocateTarget& Target = OutTargets.AddDefaulted_GetRef();
	Target.Kind = Kind;
	...
	Target.DisplayLabel = HCI_BuildLocateTargetLabelFromPath(
		TargetPath,
		Kind == EHCIAgentExecutionLocateTargetKind::Actor,
		ToolName,
		EvidenceKey);
}

class SHCIAgentPlanPreviewWindow final : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SHCIAgentPlanPreviewWindow)
		: _Plan()
		, _Rows()
		, _Context()
	{
	}
		SLATE_ARGUMENT(FHCIAgentPlan, Plan)
		SLATE_ARGUMENT(TArray<TSharedPtr<FHCIAgentPlanPreviewRow>>, Rows)
		SLATE_ARGUMENT(FHCIAgentPlanPreviewContext, Context)
	SLATE_END_ARGS()
	...
	void Construct(const FArguments& InArgs)
	{
		...
		const auto RunPlan = [this](const bool bDryRun, const bool bUserConfirmedWriteSteps)
		{
			...
			FHCIAgentPlanPreviewWindow::ExecutePlan(Plan, bDryRun, bUserConfirmedWriteSteps, Report);
			...
			if (ToolResultSummaryText.IsValid())
			{
				...
				HCIAgentToolResultSummaryFormatter::BuildSummaryLines(Report.StepResults, SummaryOptions, Lines);
		...
		for (const TSharedPtr<FHCIAgentPlanPreviewRow>& Row : Rows)
		{
			if (!Row.IsValid())
			{
				continue;
			...
			[
				...
				[
							...
							.OnClicked_Lambda([this, RunPlan]()
							{
								...
								if (UserDecision != EAppReturnType::Yes)
								{
									UE_LOG(
										LogHCIAgentPlanPreview,
										Display,
										TEXT("[HCI][AgentPlanPreview] mode=execute cancelled_by_user request_id=%s"),
										*Plan.RequestId);
									if (RunSummaryText.IsValid())
									{
					...
					[
						...
						.OnClicked_Lambda([WeakThis = TWeakPtr<SHCIAgentPlanPreviewWindow>(SharedThis(this))]()
						{
							if (const TSharedPtr<SHCIAgentPlanPreviewWindow> Pinned = WeakThis.Pin())
							{
...
private:
	FHCIAgentPlan Plan;
	TArray<TSharedPtr<FHCIAgentPlanPreviewRow>> Rows;
...
TArray<TSharedPtr<FHCIAgentPlanPreviewRow>> FHCIAgentPlanPreviewWindow::BuildRows(const FHCIAgentPlan& Plan)
{
	TArray<TSharedPtr<FHCIAgentPlanPreviewRow>> Rows;
	...
	for (int32 StepIndex = 0; StepIndex < Plan.Steps.Num(); ++StepIndex)
	{
		...
		TSharedPtr<FHCIAgentPlanPreviewRow> Row = MakeShared<FHCIAgentPlanPreviewRow>();
		Row->StepIndex = StepIndex;
		Row->StepId = Step.StepId;
		Row->ToolName = Step.ToolName.ToString();
		Row->RiskLevel = FHCIAgentPlanContract::RiskLevelToString(Step.RiskLevel);
...
FHCIAgentPlanCommitRiskSummary FHCIAgentPlanPreviewWindow::BuildCommitRiskSummary(const FHCIAgentPlan& Plan)
{
	FHCIAgentPlanCommitRiskSummary Summary;
...
FString FHCIAgentPlanPreviewWindow::BuildCommitConfirmMessage(const FHCIAgentPlan& Plan)
{
	const FHCIAgentPlanCommitRiskSummary Summary = BuildCommitRiskSummary(Plan);
...
FString FHCIAgentPlanPreviewWindow::BuildSearchPathEvidenceSummary(const TArray<FHCIAgentExecutorStepResult>& StepResults)
{
	for (const FHCIAgentExecutorStepResult& StepResult : StepResults)
	{
...
void FHCIAgentPlanPreviewWindow::BuildLocateTargetsFromStepResults(
	...
	TArray<FHCIAgentExecutionLocateTarget>& OutTargets)
{
	...
	auto AddActorEvidenceList = [&](const FHCIAgentExecutorStepResult& StepResult, const TCHAR* EvidenceKey)
	{
		...
		for (const FString& ActorPath : Tokens)
		{
			HCI_AddLocateTargetUnique(
				EHCIAgentExecutionLocateTargetKind::Actor,
				ActorPath,
				FString(),
				StepResult.ToolName,
				EvidenceKey,
	...
	auto AddAssetEvidenceList = [&](const FHCIAgentExecutorStepResult& StepResult, const TCHAR* EvidenceKey, const TCHAR* Delimiter = TEXT(" | "))
	{
		const FString* Raw = StepResult.Evidence.Find(EvidenceKey);
		if (Raw == nullptr)
		{
		...
		{
			HCI_AddLocateTargetUnique(
				EHCIAgentExecutionLocateTargetKind::Asset,
	...
	auto AddSingleAssetEvidence = [&](const FHCIAgentExecutorStepResult& StepResult, const TCHAR* EvidenceKey)
	{
		const FString* Raw = StepResult.Evidence.Find(EvidenceKey);
		if (Raw == nullptr)
		{
...
bool FHCIAgentPlanPreviewWindow::ExecutePlan(
	const FHCIAgentPlan& Plan,
	const bool bDryRun,
	const bool bUserConfirmedWriteSteps,
	FHCIAgentPlanExecutionReport& OutReport,
	TFunction<void(int32, int32, const FHCIAgentPlanStep&)> OnStepBegin)
{
	OutReport = FHCIAgentPlanExecutionReport();
	...
	if (OnStepBegin)
	{
		...
		Options.OnStepBegin = [OnStepBegin, TotalSteps](const int32 StepIndex, const FHCIAgentPlanStep& Step)
		{
			OnStepBegin(StepIndex, TotalSteps, Step);
		};
	}
	HCIAgentToolActions::BuildStageIDraftActions(Options.ToolActions);

	// Business-level Undo: one approved commit == one undo record (Context=HCI).
	// Keep DryRun free of transactions.
	TUniquePtr<FScopedTransaction> BusinessTransaction;
	if (!bDryRun && bUserConfirmedWriteSteps)
	{
		const FString SessionName = FString::Printf(TEXT("HCI: %s (%s)"), *Plan.Intent, *Plan.RequestId);
		BusinessTransaction = MakeUnique<FScopedTransaction>(TEXT("HCI"), FText::FromString(SessionName), nullptr, true);
	}

	FHCIAgentExecutorRunResult RunResult;
	OutReport.bRunOk = FHCIAgentExecutor::ExecutePlan(
		Plan,
		ToolRegistry,
	...
	for (const FHCIAgentExecutorStepResult& Step : RunResult.StepResults)
	{
	...
	UE_LOG(
		LogHCIAgentPlanPreview,
		Display,
		TEXT("[HCI][AgentPlanPreview] mode=%s execution_mode=%s executed request_id=%s steps=%d terminal=%s terminal_reason=%s succeeded=%d failed=%d scanned_assets=%d scanned_level_actors=%d risky_level_actors=%d"),
	...
	if (OutReport.LocateTargets.Num() > 0)
	{
		UE_LOG(
			LogHCIAgentPlanPreview,
			Display,
			TEXT("[HCI][AgentPlanPreview] locate_targets=%d request_id=%s"),
...
void FHCIAgentPlanPreviewWindow::OpenWindow(const FHCIAgentPlan& Plan, const FHCIAgentPlanPreviewContext& Context)
{
	const TArray<TSharedPtr<FHCIAgentPlanPreviewRow>> Rows = BuildRows(Plan);

	TSharedRef<SWindow> Window = SNew(SWindow)
		.Title(FText::FromString(TEXT("HCI Plan Preview (Stage I3 Draft)")))
		.ClientSize(FVector2D(1080.0f, 680.0f))
		.SupportsMaximize(true)
		.SupportsMinimize(true);

	Window->SetContent(
		SNew(SHCIAgentPlanPreviewWindow)
		.Plan(Plan)
		.Rows(Rows)
		.Context(Context));
