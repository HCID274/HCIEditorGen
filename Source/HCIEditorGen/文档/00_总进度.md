...
- 切片编号：`Stage E - Slice E2`
- 当前状态：`E1 已获用户 Pass；E2 待启动`
- 切片目标：Dry-Run Diff 契约与 UE 面板展示（Diff 结构 + 列表项证据 + 定位入口）。
- 通过后动作：进入 `Stage E - Slice E3`（确认门禁 + 事务/SourceControl 安全前置）。
