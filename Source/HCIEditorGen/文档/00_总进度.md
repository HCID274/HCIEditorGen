...
- 切片编号：`Stage D - Slice D1`
- 当前状态：`C4 已获用户 Pass；D1 待启动`
- 切片目标：深度检查批次策略与分批加载释放（`FStreamableManager`/等效分批加载，提取后释放强引用）。
- 通过后动作：进入 `Stage D - Slice D2`（GC 节流策略）。
