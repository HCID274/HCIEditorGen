...
bool FHCIAuditReportJsonSerializer::SerializeToJsonString(const FHCIAuditReport& Report, FString& OutJsonText)
{
	TSharedRef<FJsonObject> Root = MakeShared<FJsonObject>();
	Root->SetStringField(TEXT("run_id"), Report.RunId);
	Root->SetStringField(TEXT("generated_utc"), FHCITimeFormat::FormatUtcAsBeijingIso8601(Report.GeneratedUtc));
	...
	for (const FHCIAuditResultEntry& Result : Report.Results)
	{
