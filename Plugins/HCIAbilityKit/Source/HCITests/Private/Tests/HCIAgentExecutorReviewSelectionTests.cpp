...
namespace
{
static FHCIDryRunDiffReport MakeF8ReviewReport()
{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FHCIAgentExecutorReviewSelectionDedupTest,
	"HCI.Editor.AgentExecutorReviewSelect.SelectRowsDedupPreservesOrder",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FHCIAgentExecutorReviewSelectionDedupTest::RunTest(const FString& Parameters)
{
	const FHCIDryRunDiffReport Report = MakeF8ReviewReport();
	const TArray<int32> RequestedRows = {1, 1, 0};

	FHCIDryRunDiffSelectionResult Result;
	TestTrue(TEXT("Selection should succeed"), FHCIDryRunDiffSelection::SelectRows(Report, RequestedRows, Result));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FHCIAgentExecutorReviewSelectionOutOfRangeTest,
	"HCI.Editor.AgentExecutorReviewSelect.SelectRowsRejectsOutOfRange",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FHCIAgentExecutorReviewSelectionOutOfRangeTest::RunTest(const FString& Parameters)
{
	const FHCIDryRunDiffReport Report = MakeF8ReviewReport();

	FHCIDryRunDiffSelectionResult Result;
	TestFalse(TEXT("Selection should fail on out-of-range row"), FHCIDryRunDiffSelection::SelectRows(Report, {9}, Result));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FHCIAgentExecutorReviewSelectionJsonFieldsTest,
	"HCI.Editor.AgentExecutorReviewSelect.SelectedJsonKeepsLocateAndSkipReasonFields",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FHCIAgentExecutorReviewSelectionJsonFieldsTest::RunTest(const FString& Parameters)
{
	const FHCIDryRunDiffReport Report = MakeF8ReviewReport();

	FHCIDryRunDiffSelectionResult Result;
	TestTrue(TEXT("Selection should succeed"), FHCIDryRunDiffSelection::SelectRows(Report, {1}, Result));

	FString JsonText;
	TestTrue(TEXT("Serialize selected report"), FHCIDryRunDiffJsonSerializer::SerializeToJsonString(Result.SelectedReport, JsonText));
