...
|               `-- Private/
|                   |-- HCIAbilityKitEditorModule.cpp        # 启动注册 Python Hook + Reimport 菜单 + Search/AuditScan/AuditScanAsync(+Stop/Retry) 控制台命令
|                   |-- Audit/
...
|                   `-- Tests/
|                       |-- HCIAbilityKitAuditScanAsyncControllerTests.cpp # StageB-B4 自动化测试（Cancel/Retry 与 FailureConvergence）
|                       `-- HCIAbilityKitAuditRuleRegistryTests.cpp # StageC-C1 自动化测试（Mismatch 触发与缺失 expected 不触发）
