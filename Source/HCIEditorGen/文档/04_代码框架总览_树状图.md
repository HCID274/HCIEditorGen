...
|           |   |   |-- Agent/
|           |   |   |   |-- HCIAbilityKitAgentExecutionGate.h # StageE-E3 确认门禁契约（执行步 -> 门禁决策）
...
|           |       |-- Agent/
|           |       |   |-- HCIAbilityKitAgentExecutionGate.cpp # StageE-E3 确认门禁 helper（未确认写操作拦截）
...
|               `-- Private/
|                   |-- HCIAbilityKitEditorModule.cpp        # 启动注册 Python Hook + Reimport 菜单 + Search/AuditScan/AuditExportJson/AuditScanAsync(+Stop/Retry)+ToolRegistryDump+DryRunDiffPreview+AgentConfirmGateDemo 控制台命令
|                   |-- Audit/
...
|                       |-- HCIAbilityKitAgentExecutionGateTests.cpp # StageE-E3 自动化测试（确认门禁：未确认写阻断/只读放行/确认写放行）
