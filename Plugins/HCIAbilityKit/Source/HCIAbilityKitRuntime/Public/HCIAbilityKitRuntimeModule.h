 ...
 * 负责插件运行时核心逻辑的加载与卸载。
 */
class HCIABILITYKITRUNTIME_API FHCIAbilityKitRuntimeModule : public IModuleInterface
{
public:
	static FHCIAbilityKitRuntimeModule& Get();
	 ...
	 */
	void RegisterPlannerRouter(const TSharedRef<IHCIAbilityKitPlannerRouter>& InRouter);

	/** Get the active planner router (will lazily create a default router if missing). */
	TSharedRef<IHCIAbilityKitPlannerRouter> GetPlannerRouter();

	/** Create the default router wired with default providers (no statics; caller may register it). */
	TSharedRef<IHCIAbilityKitPlannerRouter> CreateDefaultPlannerRouter();
...
private:
	...
	TSharedPtr<IHCIAbilityKitPlannerRouter> PlannerRouter;
};
