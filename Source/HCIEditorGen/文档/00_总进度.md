...
- 切片编号：`Stage B - Slice B5`
- 当前状态：`B4 已获用户 Pass；B5 待启动`
- 切片目标：采集 `triangle_count` 相关 Tags（若存在），标记 `triangle_source=tag_cached`。
- 通过后动作：进入 `Stage B - Slice B6`（Locked/Dirty 状态过滤与跳过留痕）。
