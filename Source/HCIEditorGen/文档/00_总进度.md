...
- 切片编号：`Stage B - Slice B8`
- 当前状态：`B7 已获用户 Pass；B8 待启动`
- 切片目标：预览体自动同步（`PostEditChangeProperty` 响应资产/路径变更；Reimport 后自动刷新显示网格）。
- 通过后动作：进入 `Stage C - Slice C1`（RuleRegistry 框架与规则接口落地）。
