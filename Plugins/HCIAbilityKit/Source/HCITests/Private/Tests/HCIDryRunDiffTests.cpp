...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FHCIDryRunDiffNormalizeLocateStrategyTest,
	"HCI.Editor.AgentDryRun.NormalizeLocateStrategy",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FHCIDryRunDiffNormalizeLocateStrategyTest::RunTest(const FString& Parameters)
{
	FHCIDryRunDiffReport Report;
	Report.RequestId = TEXT("req_test_e2_001");

	FHCIDryRunDiffItem& AssetItem = Report.DiffItems.Emplace_GetRef();
	...
	AssetItem.Risk = EHCIDryRunRisk::Write;

	FHCIDryRunDiffItem& ActorItem = Report.DiffItems.Emplace_GetRef();
	...
	ActorItem.Risk = EHCIDryRunRisk::ReadOnly;
	...
	TestEqual(
		...
		FHCIDryRunDiff::LocateStrategyToString(AssetItem.LocateStrategy),
	...
	TestEqual(
		...
		FHCIDryRunDiff::LocateStrategyToString(ActorItem.LocateStrategy),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FHCIDryRunDiffJsonSerializerCoreFieldsTest,
	"HCI.Editor.AgentDryRun.JsonSerializerIncludesCoreFields",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FHCIDryRunDiffJsonSerializerCoreFieldsTest::RunTest(const FString& Parameters)
{
	FHCIDryRunDiffReport Report;
	Report.RequestId = TEXT("req_test_e2_002");

	FHCIDryRunDiffItem& Item = Report.DiffItems.Emplace_GetRef();
	...
	FString JsonText;
	const bool bOk = FHCIDryRunDiffJsonSerializer::SerializeToJsonString(Report, JsonText);
