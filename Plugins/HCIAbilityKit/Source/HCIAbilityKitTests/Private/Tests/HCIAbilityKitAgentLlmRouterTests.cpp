...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FHCIAbilityKitLlmRouterDropsBelow50SuccessRateTest,
	"HCIAbilityKit.Editor.AgentPlanLLM.RouterDropsModelsBelow50PercentSuccessRate",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FHCIAbilityKitLlmRouterDropsBelow50SuccessRateTest::RunTest(const FString& Parameters)
{
	FHCIAbilityKitAgentLlmClient::ResetRoutingStateForTesting();

	const FString BaseDir = FPaths::ConvertRelativePathToFull(
		FPaths::ProjectSavedDir() / TEXT("Automation/HCIAbilityKit/LlmRouterDropsBelow50"));
	...
	{
		FHCIAbilityKitAgentLlmProviderConfig Config;
		FString Error;
		const bool bLoaded = FHCIAbilityKitAgentLlmClient::LoadProviderConfigFromJsonFile(
			ProviderPath,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FHCIAbilityKitLlmRouterSmoothWrrDistributionTest,
	"HCIAbilityKit.Editor.AgentPlanLLM.RouterUsesSmoothWeightedRoundRobin",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FHCIAbilityKitLlmRouterSmoothWrrDistributionTest::RunTest(const FString& Parameters)
{
	FHCIAbilityKitAgentLlmClient::ResetRoutingStateForTesting();

	const FString BaseDir = FPaths::ConvertRelativePathToFull(
		FPaths::ProjectSavedDir() / TEXT("Automation/HCIAbilityKit/LlmRouterSmoothWrr"));
	const FString ProviderPath = BaseDir / TEXT("llm_provider.local.json");
	const FString RouterPath = BaseDir / TEXT("llm_router.local.json");
	const FString JsonRouterPath = HCI_NormalizeJsonPath(RouterPath);
