	...
	FString& OutFailureReason)
{
	...
	{
		OutBaseName = HCIAssetNamingRules::SanitizeIdentifier(HCIAssetNamingRules::RemoveKnownPrefixToken(SourceStem));
		if (!OutBaseName.IsEmpty())
		{
			if (OutBaseName.StartsWith(Prefix + TEXT("_"), ESearchCase::IgnoreCase))
			{
				...
				OutBaseName = HCIAssetNamingRules::SanitizeIdentifier(OutBaseName);
			}
			if (!OutBaseName.IsEmpty())
			{
				return true;
...
static FString HCI_DeriveBaseNameFromCurrentName(const FString& CurrentAssetName, const FString& Prefix)
{
	...
	Candidate = HCIAssetNamingRules::RemoveKnownPrefixToken(Candidate);

	FString Sanitized = HCIAssetNamingRules::SanitizeIdentifier(Candidate);
	if (!Sanitized.IsEmpty())
	{
		return Sanitized;
	}
	return HCIAssetNamingRules::SanitizeIdentifier(CurrentAssetName);
}
...
static FHCIAssetRoutingRules HCI_LoadAssetRoutingRules()
{
	FHCIAssetRoutingRules Rules;
	...
	if (Root->TryGetStringField(TEXT("shared_root"), SharedRoot))
	{
		...
		SharedRoot = HCIAssetNamingRules::SanitizeIdentifier(SharedRoot);
	...
	if (Root->TryGetObjectField(TEXT("subfolders"), Subfolders) && Subfolders && Subfolders->IsValid())
	{
		for (const auto& It : (*Subfolders)->Values)
		{
			...
			Val = HCIAssetNamingRules::SanitizeIdentifier(Val);
