...
|           |   |   |-- Agent/
|           |   |   |   |-- HCIAbilityKitAgentExecutionGate.h # StageE-E3/E4 执行安全门禁契约（确认门禁 + Blast Radius 限流）
...
|           |       |-- Agent/
|           |       |   |-- HCIAbilityKitAgentExecutionGate.cpp # StageE-E3/E4 执行安全门禁 helper（确认门禁 + MAX_ASSET_MODIFY_LIMIT=50）
...
|               `-- Private/
|                   |-- HCIAbilityKitEditorModule.cpp        # 启动注册 Python Hook + Reimport 菜单 + Search/AuditScan/AuditExportJson/AuditScanAsync(+Stop/Retry)+ToolRegistryDump+DryRunDiffPreview+AgentConfirmGate/BlastRadius/Transaction/SourceControl/RbacDemo 控制台命令
|                   |-- Audit/
...
|                       |-- HCIAbilityKitAgentExecutionGateTests.cpp # StageE-E3~E7 自动化测试（Confirm/BlastRadius/Transaction/SourceControl/MockRBAC/本地日志）
