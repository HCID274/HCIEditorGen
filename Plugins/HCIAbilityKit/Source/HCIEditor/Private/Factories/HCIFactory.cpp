...
// 静态日志分类定义
DEFINE_LOG_CATEGORY_STATIC(LogHCIFactory, Log, All);
...
{
#if WITH_EDITOR
	const FString AssetLabel = Asset ? Asset->GetName() : TEXT("HCI");
...
void RefreshPreviewActorsBoundToAsset(const UHCIAsset* ChangedAsset, const FString& Source)
{
#if WITH_EDITOR
	if (!ChangedAsset)
	{
	...
	for (TObjectIterator<AHCIPreviewActor> It; It; ++It)
	{
		AHCIPreviewActor* PreviewActor = *It;
	...
	if (RefreshedActorCount > 0)
	{
		UE_LOG(
			LogHCIFactory,
			Display,
			TEXT("[HCI][PreviewSync] source=%s asset=%s refreshed_actors=%d"),
			*Source,
	...
	FHCIParseError& OutError)
{
	...
	{
		OutError.Code = HCIErrorCodes::InvalidObjectPath;
...
bool UHCIFactory::FactoryCanImport(const FString& Filename)
{
	// 基于文件扩展名判断是否可以进行导入
	return FPaths::GetExtension(Filename, true).Equals(TEXT(".hciabilitykit"), ESearchCase::IgnoreCase);
}

UObject* UHCIFactory::FactoryCreateFile(
	UClass* InClass,
	UObject* InParent,
	FName InName,
	EObjectFlags Flags,
	...
	bool& bOutOperationCanceled)
{
	...
	FHCIParsedData ParsedData;
	FHCIParseError ParseError;
	
	// 调用解析服务解析源文件内容
	if (!FHCIParserService::TryParseKitFile(Filename, ParsedData, ParseError))
	{
		const FString ErrorMsg = ParseError.ToContractString();
		if (Warn)
		{
			Warn->Logf(ELogVerbosity::Error, TEXT("HCI import failed: %s"), *ErrorMsg);
		}
		UE_LOG(LogHCIFactory, Error, TEXT("Import failed: %s"), *ErrorMsg);
		
		if (!ParseError.Detail.IsEmpty())
		{
			UE_LOG(LogHCIFactory, Error, TEXT("Import detail: %s"), *ParseError.Detail);
		}
	...
	// 成功解析后创建新的资产对象
	UHCIAsset* NewAsset = NewObject<UHCIAsset>(InParent, InClass, InName, Flags);
	if (!NewAsset)
	{
		ShowImportFailureNotification(TEXT("Failed to create UHCIAsset object"));
		bOutOperationCanceled = true;
		return nullptr;
	}
	...
	FHCISearchIndexService::Get().RefreshAsset(NewAsset);
	RefreshPreviewActorsBoundToAsset(NewAsset, TEXT("import"));
	return NewAsset;
}

bool UHCIFactory::CanReimport(UObject* Obj, TArray<FString>& OutFilenames)
{
	UHCIAsset* Asset = Cast<UHCIAsset>(Obj);
	if (!Asset)
	{
		return false;
	}
...
void UHCIFactory::SetReimportPaths(UObject* Obj, const TArray<FString>& NewReimportPaths)
{
	UHCIAsset* Asset = Cast<UHCIAsset>(Obj);
...
EReimportResult::Type UHCIFactory::Reimport(UObject* Obj)
{
	UHCIAsset* Asset = Cast<UHCIAsset>(Obj);
	if (!Asset)
	{
		ShowReimportFailureNotification(Obj, TEXT("Selected object is not UHCIAsset"));
	...
	{
		FHCIParseError ParseError;
		ParseError.Code = HCIErrorCodes::FileReadError;
		...
		const FString ErrorMsg = ParseError.ToContractString();
		UE_LOG(LogHCIFactory, Error, TEXT("Reimport failed: %s"), *ErrorMsg);
	...
	FHCIParseError ParseError;
	// 重新调用解析逻辑处理最新的源文件内容
	if (!FHCIParserService::TryParseKitFile(FilenameToLoad, ParsedData, ParseError))
	{
		const FString ErrorMsg = ParseError.ToContractString();
		UE_LOG(LogHCIFactory, Error, TEXT("Reimport failed: %s"), *ErrorMsg);
		if (!ParseError.Detail.IsEmpty())
		{
			UE_LOG(LogHCIFactory, Error, TEXT("Reimport detail: %s"), *ParseError.Detail);
		}
		ShowReimportFailureNotification(Asset, ErrorMsg);
		return EReimportResult::Failed;
	}
	...
	FHCISearchIndexService::Get().RefreshAsset(Asset);
...
void UHCIFactory::ApplyParsedToAsset(UHCIAsset* Asset, const FHCIParsedData& Parsed)
{
	if (!Asset)
	{
		return;
