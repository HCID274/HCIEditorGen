...
namespace
{
class FHCIMoveAssetToolAction final : public IHCIAgentToolAction
{
public:
		...
		FHCIAgentToolActionResult& OutResult) const override
	{
		...
		const bool bMoved = HCIToolActionMoveRenameSupport::MoveAssetWithAssetTools(SourceAssetPath, DestinationAssetPath);
...
TSharedPtr<IHCIAgentToolAction> HCIToolActionFactories::MakeMoveAssetToolAction()
{
	return MakeShared<FHCIMoveAssetToolAction>();
}
