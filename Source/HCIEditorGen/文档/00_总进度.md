...
- 切片编号：`Stage F - Slice F6`
- 当前状态：`F6 已通过（用户 UE 手测 Pass）`
- 切片目标：`Executor -> Dry-Run Diff` 审阅桥接：把 `RunResult(step_results[])` 转换为 `DryRunDiffReport`，复用 E2 的 `JSON` 契约与定位策略（`actor -> camera_focus` / `asset -> sync_browser`），保持 `simulate_dry_run` 不触发真实资产写入。
- 通过后动作：已切换到 `Stage F - Slice F7`（待定义/待冻结），继续 F 段联调收尾。
