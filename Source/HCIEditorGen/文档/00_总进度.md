...
- 切片编号：`Step2-Slice1`
- 当前状态：`已开发，待UE验证`
- 切片目标：将 `.hciabilitykit` 解析/校验逻辑从 `UHCIAbilityKitFactory` 抽离到 Runtime 服务层，Factory 只做编排与资产写入。
- 验收动作（UE）：
  - 打开工程并编译 Editor；
  - 使用合法 `.hciabilitykit` 执行 Import/Reimport，确认资产值更新正常；
  - 使用非法 `.hciabilitykit` 执行 Reimport，确认失败通知与错误原因仍然可见；
  - 确认失败路径不脏写。
- 通过后动作：进入 Step2-Slice2（引入 Python 驱动的解析入口骨架）。
