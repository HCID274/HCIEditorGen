...
- 切片编号：`Step2-Slice2`
- 当前状态：`已开发，待UE验证`
- 切片目标：引入 UE 内置 Python 调用入口，建立“Factory -> Runtime Service -> Python”最小可用链路（先打通，不引入 LLM）。
- 验收动作（UE）：
  - 打开工程并编译 Editor；
  - 执行一次 Reimport，确认链路中 Python 入口被调用且不影响现有结果；
  - 在源文件中加入 `__force_python_fail__: true`，确认 Python 失败可见且不脏写；
  - 移除失败标记后再次 Reimport，确认恢复成功。
- 通过后动作：进入 Step2-Slice3（把一部分字段映射切到 Python 结果驱动）。
