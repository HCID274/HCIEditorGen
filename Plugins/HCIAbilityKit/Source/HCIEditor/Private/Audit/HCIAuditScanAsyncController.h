...
enum class EHCIAuditScanAsyncPhase : uint8
{
	Idle,
	Running,
	Completed,
...
class HCIEDITOR_API FHCIAuditScanAsyncController
{
public:
	...
	FHCIAuditScanBatch DequeueBatch();

	void Complete();
	void Fail(const FString& InFailureReason);
	void Reset(bool bClearRetryContext);

	bool IsRunning() const { return Phase == EHCIAuditScanAsyncPhase::Running; }
	bool CanRetry() const { return RetryAssetDatas.Num() > 0 && Phase != EHCIAuditScanAsyncPhase::Running; }
	...
	EHCIAuditScanAsyncPhase GetPhase() const { return Phase; }
...
private:
	...
	EHCIAuditScanAsyncPhase Phase = EHCIAuditScanAsyncPhase::Idle;
	int32 BatchSize = 256;
	int32 LogTopN = 10;
	int32 NextIndex = 0;
