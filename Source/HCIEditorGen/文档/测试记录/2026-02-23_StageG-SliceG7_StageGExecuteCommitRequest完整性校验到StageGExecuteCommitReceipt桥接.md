...
- Tests：新增 `HCI.Editor.AgentExecutorStageGExecuteCommitReceipt` 自动化测试。
...
2. 执行正常场景：`HCI.AgentExecutePlanReviewPrepareStageGExecuteCommitReceipt none`。
3. 执行 JSON 输出：`HCI.AgentExecutePlanReviewPrepareStageGExecuteCommitReceiptJson none`。
4. 执行阻断场景：
