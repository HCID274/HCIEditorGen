...
{
	FHCIAbilityKitValidateAssetExistsResult Result;
	Result.bOk = false;
...
bool MoveAssetWithAssetTools(const FString& SourceAssetPath, const FString& DestinationAssetPath)
{
	...
	if (!HCIAbilityKitAssetPathUtils::TrySplitObjectPath(DestinationAssetPath, DestinationPackagePath, DestinationAssetName))
	{
		return false;
	}
