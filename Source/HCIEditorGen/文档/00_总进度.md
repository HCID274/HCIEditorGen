...
- 切片编号：`Stage F - Slice F8`
- 当前状态：`F8 已通过（用户 UE 手测 Pass）`
- 切片目标：`ExecutorReview` 逐项采纳选择契约与过滤预览：在 F6/F7 的“最新审阅预览”上按 `row_index` 列表执行去重/越界校验/过滤，生成“已采纳子集” `DryRunDiffReport` 与 JSON 预览，保持 `simulate_dry_run` 不触发真实资产写入。
- 通过后动作：已切换到 `Stage F - Slice F9`（待定义/待冻结），继续 F 段联调收尾。
