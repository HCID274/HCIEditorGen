...
{
	const FString OutDir = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("HCIAbilityKit/TestFixtures/MatLink"));
	IFileManager::Get().MakeDirectory(*OutDir, true);
	...
	const FString OutPath = FPaths::Combine(OutDir, TEXT("latest.json"));
	FFileHelper::SaveStringToFile(JsonText, *OutPath);
	UE_LOG(LogHCIAbilityKitFixtures, Display, TEXT("[HCIAbilityKit][Fixtures] wrote_matlink_report path=%s bytes=%d"), *OutPath, JsonText.Len());
}
	...
	FString& OutGroupHint)
{
	...
	{
		UE_LOG(LogHCIAbilityKitFixtures, Warning, TEXT("[HCIAbilityKit][Fixtures] matlink_no_candidate meshes=%d (need at least: mesh with 2 textures + material-like deps)"), SeedMeshes.Num());
		return false;
	}
	...
	if (!BestMaster.IsValid())
	{
		UE_LOG(LogHCIAbilityKitFixtures, Warning, TEXT("[HCIAbilityKit][Fixtures] matlink_master_not_found mesh=%s (will fail; Seed may not include any Material assets referenced by the mesh)"), *BestMesh.GetObjectPathString());
		return false;
	}
...
{
	const FString InPath = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("HCIAbilityKit/TestFixtures/MatLink/latest.json"));
	FString JsonText;
...
static void HCI_StartMatLinkJob(const EHCIMatLinkJobKind Kind, const EHCIMatLinkJobStage Stage, TArray<FHCIMatLinkDupTask>&& Tasks)
{
	if (GMatLinkJob.IsActive())
	{
		UE_LOG(LogHCIAbilityKitFixtures, Warning, TEXT("[HCIAbilityKit][Fixtures] matlink_job_busy kind=%d stage=%d"), static_cast<int32>(GMatLinkJob.Kind), static_cast<int32>(GMatLinkJob.Stage));
		return;
	}
...
static void HCI_RunMatLinkBuildSnapshotCommand(const TArray<FString>& Args)
{
	...
	{
		UE_LOG(LogHCIAbilityKitFixtures, Error, TEXT("[HCIAbilityKit][Fixtures] matlink_select_failed seed=%s"), HCI_SeedRoot);
		return;
	}
	...
	UE_LOG(LogHCIAbilityKitFixtures, Display, TEXT("[HCIAbilityKit][Fixtures] matlink_build_snapshot select anchor=%s master=%s group_hint=%s roles=%d"),
...
static void HCI_RunMatLinkResetCommand(const TArray<FString>& Args)
{
	...
	{
		UE_LOG(LogHCIAbilityKitFixtures, Error, TEXT("[HCIAbilityKit][Fixtures] matlink_snapshot_missing (run HCIAbilityKit.MatLinkBuildSnapshot first) snapshot=%s masters=%s"),
	...
	{
		UE_LOG(LogHCIAbilityKitFixtures, Error, TEXT("[HCIAbilityKit][Fixtures] matlink_snapshot_assets_missing mesh=%s master=%s"), *SnapshotMeshAssetPath, *SnapshotMasterAssetPath);
		return;
	}
	...
	UE_LOG(LogHCIAbilityKitFixtures, Display, TEXT("[HCIAbilityKit][Fixtures] matlink_reset started chaos=%s clean=%s scenarios=3 group_hint=%s"),
...
void FHCIAbilityKitAgentDemoConsoleCommands::StartupFixtureCommands()
{
	...
	if (!MatLinkBuildSnapshotCommand.IsValid())
	{
		MatLinkBuildSnapshotCommand = MakeUnique<FAutoConsoleCommand>(
			TEXT("HCIAbilityKit.MatLinkBuildSnapshot"),
			TEXT("Stage O fixtures: build MatLink snapshot + master material under /Game/__HCI_Test from /Game/Seed (read-only seed). Usage: HCIAbilityKit.MatLinkBuildSnapshot"),
