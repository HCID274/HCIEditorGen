...
FString FHCIAbilityKitIngestManifest::BuildEnvContextSnippet(const int32 MaxFileNames) const
{
	TArray<FString> Names;
	Names.Reserve(Files.Num());
	for (const FHCIAbilityKitIngestManifestFile& F : Files)
	{
...
bool FHCIAbilityKitIngestManifest::TryFindLatestManifestUnderRoot(
...
bool FHCIAbilityKitIngestManifest::TryLoadFromFile(
	...
	FString& OutError)
{
	...
	for (const TSharedPtr<FJsonValue>& V : *FilesArr)
	{
		...
		FHCIAbilityKitIngestManifestFile File;
