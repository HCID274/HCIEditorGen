...
- 切片编号：`Stage F - Slice F9`
- 当前状态：`F9 已通过（用户 UE 手测 Pass）`
- 切片目标：`ExecutorReview` 采纳子集 -> `ApplyRequest` 执行申请包桥接：将 F8 输出的“已采纳子集”审阅预览桥接为可确认、可追踪的执行申请契约与 JSON（含 `selection_digest/ready/blocked_rows` 等收敛字段），为后续确认执行提供稳定输入；保持 `simulate_dry_run`，不触发真实资产写入。
- 通过后动作：已切换到 `Stage F - Slice F10`（待定义），继续 F 段收尾联调。
