...
static void HCI_AppendIfSpace(TArray<FString>& OutLines, const FString& Line, const FHCIToolResultSummaryOptions& Options)
{
...
static void HCI_FormatScanMeshTriangleCount(
	const FHCIAgentExecutorStepResult& Step,
	...
	TArray<FString>& OutLines)
{
	...
	for (const FHCIAgentExecutorStepResult& Step : StepResults)
	{
		if (Options.MaxLines > 0 && OutLines.Num() >= Options.MaxLines)
		{
			break;
