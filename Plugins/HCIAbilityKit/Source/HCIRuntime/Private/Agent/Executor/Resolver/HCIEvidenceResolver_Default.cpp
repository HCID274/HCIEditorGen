...
static bool HCI_ResolveJsonValueInternal(
	...
	FHCIEvidenceResolveError& OutError)
{
	if (!InValue.IsValid())
	{
		OutValue = InValue;
...
bool FHCIEvidenceResolver_Default::StepArgsMayContainTemplates(const TSharedPtr<FJsonObject>& Args) const
{
	return HCI_JsonObjectContainsVariableTemplate(Args);
}

bool FHCIEvidenceResolver_Default::ResolveStepArgs(
	const FHCIAgentPlanStep& InStep,
	const IHCIEvidenceContextView& EvidenceContext,
	FHCIAgentPlanStep& OutResolvedStep,
	FHCIEvidenceResolveError& OutError) const
{
