...
- 切片编号：`Stage B - Slice B7`
- 当前状态：`B6 已获用户 Pass；B7 待启动`
- 切片目标：数据驱动预览体（Blueprint/C++ Actor）读取 `RepresentingMesh` 并支持拖入场景可视化验证。
- 通过后动作：进入 `Stage B - Slice B8`（`PostEditChangeProperty` 自动同步与 Reimport 刷新）。
