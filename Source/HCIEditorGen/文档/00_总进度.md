...
- 切片编号：`Stage C - Slice C1`
- 当前状态：`B8 已获用户 Pass；C1 待启动`
- 切片目标：RuleRegistry 框架与规则接口落地（统一注册与执行契约）。
- 通过后动作：进入 `Stage C - Slice C2`（首批规则：Texture NPOT / HighPolyAutoLOD）。
