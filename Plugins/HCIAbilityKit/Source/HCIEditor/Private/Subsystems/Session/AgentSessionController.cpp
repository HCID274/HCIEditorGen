...
// Moved: heavy session logic lives in UHCIAgentSessionController (Subsystem stays as thin Facade).

#include "Agent/LLM/HCIAgentPromptBuilder.h"
#include "Agent/Presentation/HCIAgentToolResultSummaryFormatter.h"
...
namespace
{
static FHCIAgentPromptBundleOptions HCI_MakeChatSummaryPromptBundleOptions_Subsystem()
{
...
static bool HCI_LoadChatQuickCommandsFromBundle(
	TArray<FHCIAgentQuickCommand>& OutCommands,
	FString& OutError)
{
	OutCommands.Reset();
	OutError.Reset();

	const FHCIAgentPromptBundleOptions Options = HCI_MakeChatSummaryPromptBundleOptions_Subsystem();
	FString BundleDir;
	if (!FHCIAgentPromptBuilder::ResolveSkillBundleDirectory(Options, BundleDir, OutError))
	{
		return false;
	}
...
static FHCIAgentPlanPreviewContext HCI_MakePreviewContext(
	const FString& RouteReason,
	const FHCIAgentPlannerResultMetadata& PlannerMetadata)
{
	FHCIAgentPlanPreviewContext PreviewContext;
...
static bool HCI_Subsystem_IsWriteLikeRisk(const EHCIAgentPlanRiskLevel RiskLevel)
{
	return RiskLevel == EHCIAgentPlanRiskLevel::Write ||
		RiskLevel == EHCIAgentPlanRiskLevel::Destructive;
}

static const TCHAR* HCI_SessionStateToLabel(const EHCIAgentSessionState State)
{
	switch (State)
	{
	case EHCIAgentSessionState::Idle:
		return TEXT("Idle");
	case EHCIAgentSessionState::Thinking:
		return TEXT("Thinking");
	case EHCIAgentSessionState::PlanReady:
		return TEXT("PlanReady");
	case EHCIAgentSessionState::AutoExecuteReadOnly:
		return TEXT("AutoExecuteReadOnly");
	case EHCIAgentSessionState::AwaitUserConfirm:
		return TEXT("AwaitUserConfirm");
	case EHCIAgentSessionState::Executing:
		return TEXT("Executing");
	case EHCIAgentSessionState::Summarizing:
		return TEXT("Summarizing");
	case EHCIAgentSessionState::Completed:
		return TEXT("Completed");
	case EHCIAgentSessionState::Failed:
		return TEXT("Failed");
	case EHCIAgentSessionState::Cancelled:
...
static FString HCI_GetStepArgString(const FHCIAgentPlanStep& Step, const TCHAR* FieldName)
{
...
static int32 HCI_GetStepArgInt(const FHCIAgentPlanStep& Step, const TCHAR* FieldName, const int32 DefaultValue = 0)
{
	if (!Step.Args.IsValid())
	{
		return DefaultValue;
...
static FString HCI_BuildStepSummaryFallback(const FHCIAgentPlanStep& Step)
{
...
static FString HCI_BuildStepIntentReasonFallback(const FHCIAgentPlanStep& Step)
{
...
static FString HCI_BuildStepRiskWarningFallback(const FHCIAgentPlanStep& Step)
{
	...
	if (Step.RiskLevel == EHCIAgentPlanRiskLevel::Destructive)
	{
		return TEXT("高风险操作：可能删除或覆盖大量资产，请确认后执行。");
	}
	if (Step.bRequiresConfirm || Step.RiskLevel == EHCIAgentPlanRiskLevel::Write)
	{
...
static bool HCI_TryGetStepArgStringArray(const FHCIAgentPlanStep& Step, const TCHAR* FieldName, TArray<FString>& OutValues)
{
	OutValues.Reset();
	if (!Step.Args.IsValid())
	{
...
static FString HCI_BuildStepImpactHintFallback(const FHCIAgentPlanStep& Step)
{
...
static FHCIAgentUiProgressState HCI_MakeProgressState(
	const bool bVisible,
	const bool bIndeterminate,
	const float Percent01,
	const FString& Label)
{
	FHCIAgentUiProgressState State;
...
static FHCIAgentUiProgressState HCI_BuildProgressStateForSessionState(const EHCIAgentSessionState State)
{
	switch (State)
	{
	case EHCIAgentSessionState::Idle:
...
static FString HCI_BuildStepActionHintForChat(const FHCIAgentPlanStep& Step)
{
	if (Step.ToolName == TEXT("SearchPath"))
	{
		return TEXT("正在定位目录...");
...
static bool HCI_TryLoadProjectRulesMarkdown(FString& OutText, bool& bOutTruncated, FString& OutError)
{
	...
	const FString RulesPath = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("HCI/Rules/Project_Rules.md"));
...
static bool HCI_TryLoadLatestIngestManifest(FHCIIngestManifest& OutManifest, FString& OutError)
{
	OutManifest = FHCIIngestManifest();
	OutError.Reset();

	const FString IngestRoot = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("HCI/Ingest"));
	FString ManifestPath;
	if (!FHCIIngestManifest::TryFindLatestManifestUnderRoot(IngestRoot, ManifestPath, OutError))
	{
		return false;
	}

	return FHCIIngestManifest::TryLoadFromFile(ManifestPath, OutManifest, OutError);
}
...
void UHCIAgentSessionController::Initialize(UHCIAgentSubsystem& InOwner)
{
	OwnerSubsystem = &InOwner;

	CommandRegistry.Reset();
	CommandRegistry.Add(TEXT("chat_submit"), UHCIAgentCommand_ChatPlanAndSummary::StaticClass());

	ReloadQuickCommands();
	SetCurrentState(EHCIAgentSessionState::Idle);
	ClearLocateTargets();
}

void UHCIAgentSessionController::Deinitialize()
{
	...
	bHasLastPlan = false;
	LastPlan = FHCIAgentPlan();
	LastRouteReason.Reset();
	LastPlannerMetadata = FHCIAgentPlannerResultMetadata();
	CurrentState = EHCIAgentSessionState::Idle;
	CurrentProgressState = FHCIAgentUiProgressState();
	CurrentActivityHint.Reset();
	LastExecutionLocateTargets.Reset();
	OwnerSubsystem = nullptr;
}

bool UHCIAgentSessionController::SubmitChatInput(const FString& UserInput, const FString& SourceTag)
{
	...
	if (HCI_TryAugmentChatInputWithLatestIngestRoot(TrimmedInput, EffectiveInput, DetectedIngestRoot))
	{
		...
		UE_LOG(LogTemp, Display, TEXT("[HCI][AgentChatUI][Env] detected_latest_ingest_root=%s"), *DetectedIngestRoot);
	}

	{
		...
		if (bChanged)
		{
			UE_LOG(LogTemp, Display, TEXT("[HCI][AgentChatUI][NormalizeInput] before=%s after=%s"), *EffectiveInput, *Normalized);
	...
	LastPlannerMetadata = FHCIAgentPlannerResultMetadata();
	ClearLocateTargets();
	SetCurrentState(EHCIAgentSessionState::Thinking);
	SetActivityHint(TEXT("思考中..."));

	FHCIAgentCommandContext Context;
	...
	{
		SetCurrentState(EHCIAgentSessionState::Failed);
...
bool UHCIAgentSessionController::IsBusy() const
{
	return ActiveCommand != nullptr;
}

bool UHCIAgentSessionController::HasLastPlan() const
{
	return bHasLastPlan;
}

EHCIAgentSessionState UHCIAgentSessionController::GetCurrentState() const
{
	return CurrentState;
}

FString UHCIAgentSessionController::GetCurrentStateLabel() const
{
	return HCI_SessionStateToLabel(CurrentState);
}

bool UHCIAgentSessionController::CanCommitLastPlanFromChat() const
{
	return bHasLastPlan && CurrentState == EHCIAgentSessionState::AwaitUserConfirm;
}

bool UHCIAgentSessionController::CanCancelPendingPlanFromChat() const
{
	return bHasLastPlan && CurrentState == EHCIAgentSessionState::AwaitUserConfirm;
}

void UHCIAgentSessionController::GetCurrentProgressState(FHCIAgentUiProgressState& OutState) const
{
	OutState = CurrentProgressState;
}

void UHCIAgentSessionController::GetCurrentActivityHint(FString& OutHint) const
{
	OutHint = CurrentActivityHint;
}

void UHCIAgentSessionController::GetLastExecutionLocateTargets(TArray<FHCIAgentUiLocateTarget>& OutTargets) const
{
	OutTargets = LastExecutionLocateTargets;
}

bool UHCIAgentSessionController::BuildLastPlanCardLines(TArray<FString>& OutLines) const
{
	OutLines.Reset();
	if (!bHasLastPlan)
	{
	...
	if (CurrentState == EHCIAgentSessionState::AwaitUserConfirm && UHCIAgentSubsystem::IsWriteLikePlan(LastPlan))
	{
		int32 WriteStepCount = 0;
		int32 DestructiveStepCount = 0;
		int32 ReadOnlyPrepCount = 0;
		for (const FHCIAgentPlanStep& Step : LastPlan.Steps)
		{
		...
		for (const FHCIAgentPlanStep& Step : LastPlan.Steps)
		{
			...
			const FString IntentReason = UHCIAgentSubsystem::BuildStepIntentReasonForUi(Step);
			const FString RiskWarning = UHCIAgentSubsystem::BuildStepRiskWarningForUi(Step);
	...
	{
		const FHCIAgentPlanStep& Step = LastPlan.Steps[Index];
		const FString Summary = UHCIAgentSubsystem::BuildStepDisplaySummaryForUi(Step);
		const FString IntentReason = UHCIAgentSubsystem::BuildStepIntentReasonForUi(Step);
		const FString RiskWarning = UHCIAgentSubsystem::BuildStepRiskWarningForUi(Step);
		OutLines.Add(FString::Printf(
...
static int32 HCI_RiskRankForApprovalCard(const EHCIAgentPlanRiskLevel RiskLevel)
{
	switch (RiskLevel)
	{
...
bool UHCIAgentSessionController::BuildLastPlanApprovalCard(FHCIAgentUiApprovalCard& OutCard) const
{
	...
	const FHCIAgentPlanStep* BestStep = nullptr;
	int32 BestRank = -1;
	for (const FHCIAgentPlanStep& Step : LastPlan.Steps)
	{
		if (!(Step.bRequiresConfirm || HCI_Subsystem_IsWriteLikeRisk(Step.RiskLevel)))
	...
	OutCard.KeyAction = HCI_SanitizeUiTextForArtist(UHCIAgentSubsystem::BuildStepDisplaySummaryForUi(*BestStep));
	OutCard.ImpactHint = HCI_SanitizeUiTextForArtist(UHCIAgentSubsystem::BuildStepImpactHintForUi(*BestStep));
	...
	{
		const FHCIAgentExecutorStepResult* Matched = nullptr;
		const FHCIAgentExecutorStepResult* ScanStep = nullptr;
		for (const FHCIAgentExecutorStepResult& StepResult : ApprovalPreviewStepResults)
		{
	...
	if (bWriteLike && (BestStep->RiskLevel == EHCIAgentPlanRiskLevel::Write || BestStep->RiskLevel == EHCIAgentPlanRiskLevel::Destructive))
	{
		OutCard.Warning = HCI_SanitizeUiTextForArtist(UHCIAgentSubsystem::BuildStepRiskWarningForUi(*BestStep));
	}
...
bool UHCIAgentSessionController::OpenLastPlanPreview()
{
	if (!bHasLastPlan)
	{
	...
	const FHCIAgentPlanPreviewContext PreviewContext = HCI_MakePreviewContext(LastRouteReason, LastPlannerMetadata);
	FHCIAgentPlanPreviewWindow::OpenWindow(LastPlan, PreviewContext);
	EmitAssistantLine(TEXT("已打开计划预览窗口。"));
	return true;
}

bool UHCIAgentSessionController::CommitLastPlanFromChat()
{
	if (ActiveCommand != nullptr)
	{
		EmitStatus(TEXT("状态：忙碌"));
	...
	return ExecuteLastPlan(EHCIAgentPlanExecutionBranch::AwaitUserConfirm, TEXT("manual_commit"));
}

bool UHCIAgentSessionController::CancelPendingPlanFromChat()
{
	if (ActiveCommand != nullptr)
	{
		EmitStatus(TEXT("状态：忙碌"));
	...
	UE_LOG(LogTemp, Display, TEXT("[HCI][AgentChatStateMachine] trigger=manual_cancel state=await_user_confirm"));
	EmitAssistantLine(TEXT("已取消本次执行（未触发真实写操作）。"));
	SetCurrentState(EHCIAgentSessionState::Cancelled);
	return true;
}

bool UHCIAgentSessionController::TryLocateLastExecutionTargetByIndex(const int32 TargetIndex)
{
	...
	const FHCIAgentUiLocateTarget& Target = LastExecutionLocateTargets[TargetIndex];
	FString LocateReason;
	bool bLocateOk = false;
	if (Target.Kind == EHCIAgentUiLocateTargetKind::Actor)
	{
	...
	UE_LOG(
		LogTemp,
		Display,
		TEXT("[HCI][AgentChatUI][Locate] ok=%s kind=%s target=%s reason=%s"),
		bLocateOk ? TEXT("true") : TEXT("false"),
		Target.Kind == EHCIAgentUiLocateTargetKind::Actor ? TEXT("actor") : TEXT("asset"),
		*Target.TargetPath,
		*LocateReason);
...
void UHCIAgentSessionController::ReloadQuickCommands()
{
	FString Error;
...
const TArray<FHCIAgentQuickCommand>& UHCIAgentSessionController::GetQuickCommands() const
{
	return QuickCommands;
}

const FString& UHCIAgentSessionController::GetQuickCommandsLoadError() const
{
	return QuickCommandsLoadError;
}

void UHCIAgentSessionController::EmitUserLine(const FString& Text)
{
	if (UHCIAgentSubsystem* Owner = OwnerSubsystem.Get())
	{
		Owner->OnChatLine.Broadcast(FString::Printf(TEXT("你：%s"), *Text));
	}
}

void UHCIAgentSessionController::EmitAssistantLine(const FString& Text)
{
	if (UHCIAgentSubsystem* Owner = OwnerSubsystem.Get())
	{
		Owner->OnChatLine.Broadcast(FString::Printf(TEXT("系统：%s"), *Text));
	}
}

void UHCIAgentSessionController::EmitStatus(const FString& Text)
{
	if (UHCIAgentSubsystem* Owner = OwnerSubsystem.Get())
	{
		Owner->OnStatusChanged.Broadcast(Text);
	}
}

bool UHCIAgentSubsystem::IsWriteLikePlan(const FHCIAgentPlan& Plan)
{
...
EHCIAgentPlanExecutionBranch UHCIAgentSubsystem::ClassifyPlanExecutionBranch(const FHCIAgentPlan& Plan)
{
...
FString UHCIAgentSubsystem::BuildStepDisplaySummaryForUi(const FHCIAgentPlanStep& Step)
{
	return HCI_BuildStepSummaryFallback(Step);
}

FString UHCIAgentSubsystem::BuildStepIntentReasonForUi(const FHCIAgentPlanStep& Step)
{
	return HCI_BuildStepIntentReasonFallback(Step);
}

FString UHCIAgentSubsystem::BuildStepRiskWarningForUi(const FHCIAgentPlanStep& Step)
{
	return HCI_BuildStepRiskWarningFallback(Step);
}

FString UHCIAgentSubsystem::BuildStepImpactHintForUi(const FHCIAgentPlanStep& Step)
{
	return HCI_BuildStepImpactHintFallback(Step);
}

void UHCIAgentSessionController::SetProgressState(const FHCIAgentUiProgressState& InState)
{
	CurrentProgressState = InState;
	if (UHCIAgentSubsystem* Owner = OwnerSubsystem.Get())
	{
		Owner->OnProgressStateChanged.Broadcast(CurrentProgressState);
	}
}

void UHCIAgentSessionController::SetActivityHint(const FString& InHint)
{
	CurrentActivityHint = InHint;
	if (UHCIAgentSubsystem* Owner = OwnerSubsystem.Get())
	{
		Owner->OnActivityHintChanged.Broadcast(CurrentActivityHint);
	}
}

void UHCIAgentSessionController::ClearLocateTargets()
{
	LastExecutionLocateTargets.Reset();
	if (UHCIAgentSubsystem* Owner = OwnerSubsystem.Get())
	{
		Owner->OnLocateTargetsChanged.Broadcast();
	}
}

void UHCIAgentSessionController::SetLocateTargetsFromExecutionReport(const FHCIAgentPlanExecutionReport& Report)
{
	LastExecutionLocateTargets.Reset();
	...
	for (const FHCIAgentExecutionLocateTarget& Target : Report.LocateTargets)
	{
	...
	for (const FHCIAgentExecutionLocateTarget& Target : Report.LocateTargets)
	{
		...
		FHCIAgentUiLocateTarget& UiTarget = LastExecutionLocateTargets.AddDefaulted_GetRef();
		UiTarget.Kind = (Target.Kind == EHCIAgentExecutionLocateTargetKind::Actor)
			? EHCIAgentUiLocateTargetKind::Actor
			: EHCIAgentUiLocateTargetKind::Asset;
...
void UHCIAgentSessionController::SetCurrentState(const EHCIAgentSessionState NewState)
{
	CurrentState = NewState;
	if (UHCIAgentSubsystem* Owner = OwnerSubsystem.Get())
	{
		Owner->OnSessionStateChanged.Broadcast(CurrentState);
	}
...
FString UHCIAgentSessionController::BuildStateStatusText(const EHCIAgentSessionState State) const
{
	switch (State)
	{
	case EHCIAgentSessionState::Idle:
...
bool UHCIAgentSessionController::ExecuteLastPlan(
	const EHCIAgentPlanExecutionBranch Branch,
	const TCHAR* TriggerTag)
{
	...
	SetCurrentState(EHCIAgentSessionState::Executing);
	SetProgressState(HCI_MakeProgressState(
		true,
		true,
		0.60f,
		FString::Printf(TEXT("进度：执行计划（0/%d 步）..."), LastPlan.Steps.Num())));
	FHCIAgentPlanExecutionReport Report;
	const bool bRunOk = FHCIAgentPlanPreviewWindow::ExecutePlan(
		LastPlan,
		false,
		Branch == EHCIAgentPlanExecutionBranch::AwaitUserConfirm,
		Report,
		[this](const int32 StepIndex, const int32 TotalSteps, const FHCIAgentPlanStep& Step)
		{
	...
	SetCurrentState(EHCIAgentSessionState::Summarizing);
	...
	{
		HCIAgentToolResultSummaryFormatter::FHCIToolResultSummaryOptions SummaryOptions;
		SummaryOptions.MaxLines = 3;
		SummaryOptions.MaxItemsPerList = 2;
		SummaryOptions.MaxCharsPerLine = 260;

		TArray<FString> SummaryLines;
		HCIAgentToolResultSummaryFormatter::BuildSummaryLines(Report.StepResults, SummaryOptions, SummaryLines);
	...
	UE_LOG(
		LogTemp,
		Display,
		TEXT("[HCI][AgentChatStateMachine] trigger=%s branch=%s run_ok=%s terminal=%s reason=%s"),
		TriggerTag == nullptr ? TEXT("-") : TriggerTag,
		Branch == EHCIAgentPlanExecutionBranch::AwaitUserConfirm ? TEXT("await_user_confirm") : TEXT("auto_read_only"),
		bRunOk ? TEXT("true") : TEXT("false"),
		*Report.TerminalStatus,
		*Report.TerminalReason);

	SetCurrentState(bRunOk ? EHCIAgentSessionState::Completed : EHCIAgentSessionState::Failed);
	return bRunOk;
}

void UHCIAgentSessionController::HandleCommandCompleted(const FHCIAgentCommandResult& Result)
{
	ActiveCommand = nullptr;
	...
	if (bHasExecutablePlanPayload)
	{
		...
		SetCurrentState(EHCIAgentSessionState::PlanReady);
		if (UHCIAgentSubsystem* Owner = OwnerSubsystem.Get())
		{
			Owner->OnPlanReady.Broadcast();
		}
	...
	if (Result.bSuccess)
	{
		...
		if (!bHasExecutablePlanPayload)
		{
			...
			if (UHCIAgentSubsystem* Owner = OwnerSubsystem.Get())
			{
				Owner->OnSummaryReceived.Broadcast(MessageToShow);
			}
		}
	}
	...
	if (!bCanProcessPlanBranch)
	{
		...
		SetCurrentState(EHCIAgentSessionState::Completed);
		return;
	}

	const EHCIAgentPlanExecutionBranch Branch = UHCIAgentSubsystem::ClassifyPlanExecutionBranch(LastPlan);
	if (Branch == EHCIAgentPlanExecutionBranch::AutoExecuteReadOnly)
	{
		SetCurrentState(EHCIAgentSessionState::AutoExecuteReadOnly);
	...
	// For write-like plans, run a dry-run preview first so the approval card can show concrete rename/move/routing proposals.
	{
		...
		FHCIAgentPlanExecutionReport PreviewReport;
		const bool bPreviewOk = FHCIAgentPlanPreviewWindow::ExecutePlan(
			LastPlan,
			true,
			true,
			PreviewReport,
		...
		UE_LOG(
			LogTemp,
			Display,
			TEXT("[HCI][AgentChatUI][ApprovalPreview] ok=%s steps=%d terminal=%s reason=%s"),
			bPreviewOk ? TEXT("true") : TEXT("false"),
			PreviewReport.StepResults.Num(),
			*PreviewReport.TerminalStatus,
			*PreviewReport.TerminalReason);

		for (const FHCIAgentExecutorStepResult& StepResult : PreviewReport.StepResults)
		{
			UE_LOG(
				LogTemp,
				Display,
				TEXT("[HCI][AgentChatUI][ApprovalPreviewStep] step_id=%s tool=%s ok=%s error=%s reason=%s evidence_keys=%s"),
				*StepResult.StepId,
				*StepResult.ToolName,
		...
		// Provide deterministic preview summary in chat (do not depend on LLM summary timeouts).
		{
			...
			TArray<FString> SummaryLines;
			HCIAgentToolResultSummaryFormatter::BuildSummaryLines(PreviewReport.StepResults, SummaryOptions, SummaryLines);
			for (const FString& Line : SummaryLines)
			{
				EmitAssistantLine(Line);
			}
	...
	SetCurrentState(EHCIAgentSessionState::AwaitUserConfirm);
	SetActivityHint(TEXT("已生成待确认计划，请确认后执行。"));
	EmitAssistantLine(TEXT("计划包含写操作。我已生成预览，请确认后执行（可在审批卡查看具体目录分发与重命名清单）。"));
}

bool UHCIAgentSessionController::ExecuteRegisteredCommand(
	const FName& CommandName,
	const FHCIAgentCommandContext& Context)
{
	const TSubclassOf<UHCIAgentCommandBase>* CommandClass = CommandRegistry.Find(CommandName);
	if (CommandClass == nullptr || !CommandClass->Get())
	{
		return false;
	}

	UHCIAgentCommandBase* Command = NewObject<UHCIAgentCommandBase>(this, CommandClass->Get());
	if (Command == nullptr)
	{
		return false;
	}
	...
	Command->ExecuteAsync(
		Context,
		FHCIAgentCommandComplete::CreateUObject(
			this,
			&UHCIAgentSessionController::HandleCommandCompleted));
	return true;
}
