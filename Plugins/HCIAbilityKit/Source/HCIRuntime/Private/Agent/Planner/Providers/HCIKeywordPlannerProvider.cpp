	...
	FString& OutError)
{
	...
	if (bLikelyChitchatIntent)
	{
		if (bIdentityIntent)
		{
			return HCI_BuildPreparedMessageOnlyPlan(
				...
				TEXT("我是 HCI 的编辑器内资产审计与执行助理。我的职责是帮你在 UE 编辑器里做资产扫描、风险检查、批量合规修复与命名归档，并在涉及写操作时先给出确认。"),
				OutPlan,
				OutRouteReason,
				OutError);
		}
	...
	else if (bNamingIntent || bExplicitPathNamingIntent)
	{
		...
		else if (bDirectoryNamingIntent)
		{
			...
			const FString SearchKeyword = HCI_PickSearchKeywordForFolderIntent(Text);
				...
				FHCIAgentPlanStep& SearchStep = OutPlan.Steps.AddDefaulted_GetRef();
				if (!HCI_StepFromTool(
						ToolRegistry,
						TEXT("step_1_search"),
							TEXT("SearchPath"),
	...
	else if (bAssetComplianceIntent)
	{
		...
		FHCIAgentPlanStep* TextureStep = nullptr;
		...
		if (HCI_TextContainsAny(Text, {TEXT("面数"), TEXT("lod")}))
		{
			...
			FHCIAgentPlanStep& LodStep = OutPlan.Steps.AddDefaulted_GetRef();
			if (!HCI_StepFromTool(
					ToolRegistry,
					StepId,
...
const TCHAR* FHCIKeywordPlannerProvider::GetName() const
{
	return TEXT("keyword_fallback");
}

bool FHCIKeywordPlannerProvider::IsNetworkBacked() const
{
	return false;
}

bool FHCIKeywordPlannerProvider::BuildPlan(
	const FString& UserText,
	const FString& RequestId,
	const FHCIToolRegistry& ToolRegistry,
...
void FHCIKeywordPlannerProvider::BuildPlanAsync(
	const FString& UserText,
	const FString& RequestId,
	const FHCIToolRegistry& ToolRegistry,
	const FHCIAgentPlannerBuildOptions& Options,
