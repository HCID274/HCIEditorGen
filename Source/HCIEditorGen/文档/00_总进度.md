...
- 切片编号：`Stage G - Slice G1`
- 当前状态：`待定义/待冻结（Stage F 已收官）`
- 切片目标：`待定义`（建议先冻结 Stage G 真执行链路的最小入口与门禁，再开始实现）。
- 通过后动作：切换到 `Stage G - Slice G2`（待定义），继续按“文档先行 -> 实现 -> UE 门禁”推进。
