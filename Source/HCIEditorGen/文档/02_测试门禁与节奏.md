...
- 回归目标：在 `simulate_dry_run` 下完成执行就绪收口，不引入真实资产写入。
- 必测项：
  - `StageGExecutionReadinessReport` 字段完整；
  - `E4218/E4219` 阻断路径生效；
  - `user_confirmed` 门禁仍生效（`E4005`）；
  - JSON 输出与摘要字段一致。

### Stage H（LLM Planner 接入，后续）

- 回归目标：LLM 仅影响 `NL -> Plan JSON` 生成层，不绕过既有门禁链路。
- 必测项：
  - LLM 输出可被 `PlanValidator` 稳定校验；
  - 非法结构/空响应/超时可回退关键词规划器；
  - 回退路径日志可观测、可复现。

### Stage I（最小 UI，后续）

- 回归目标：用户可在面板完成“看计划 -> 看差异 -> 定位对象”闭环。
- 必测项：
  - Plan 预览正确展示步骤与风险；
  - Diff 列表与定位策略一致（`camera_focus/sync_browser`）；
  - 只读交互不触发真实写入。
...
### G_GATE_01 G10 执行就绪成功路径

- 步骤：执行 `...PrepareStageGExecutionReadiness user_confirmed=1 tamper=none`。
- 预期：`ready_for_h1_planner_integration=true`，`error_code=-`。

### G_GATE_02 G10 执行就绪阻断路径

- 步骤：分别执行 `tamper=archive`、`tamper=mode` 与 `user_confirmed=0`。
- 预期：分别命中 `E4218`、`E4219`、`E4005`。
