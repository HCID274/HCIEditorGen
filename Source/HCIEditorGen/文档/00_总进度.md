...
- 切片编号：`Stage B - Slice B3`
- 当前状态：`B2 已获用户 Pass；进入异步分片执行与进度汇报实现`
- 切片目标：将 B2 的同步扫描改造成非阻塞异步分片执行，并输出可观察进度。
- 通过后动作：进入 `Stage B - Slice B4`（任务中断/重试与失败收敛）。
