...
- 切片编号：`Stage F - Slice F10`
- 当前状态：`F10 已通过（用户 UE 手测 Pass）`
- 切片目标：`ApplyRequest` 确认前校验 -> `ConfirmRequest` 执行确认申请桥接：基于 F9 的 `ApplyRequest` 与最新审阅预览执行 `review_request_id + selection_digest + ready + user_confirmed` 校验，桥接为可确认、可追踪的 `ConfirmRequest` 契约与 JSON，为后续 F11 “确认后执行骨架（仍 dry-run）”提供稳定输入；保持 `simulate_dry_run`，不触发真实资产写入。
- 通过后动作：已切换到 `Stage F - Slice F11`（待定义），继续 F 段收尾联调。
