...
- `StageD-SliceD2` 已通过。
- 工程编译通过。
- 存在可运行 `HCI.AuditScanAsync` 的 `UHCIAbilityKitAsset` 样本（建议数量较多，便于看到多次进度与 Perf 日志）。
...
- 编译：通过（助手本地已验证）。
- 自动化：通过（助手本地已验证）。
  - `HCI.Editor.AuditScanAsync`：5/5 成功（新增 `PerfMetricsHelper`）。
  - `HCI.Editor.AuditResults`：3/3 成功（回归）。
- UE 手测：通过。
  - `HCI.AuditScanAsync 1 20 1 3` 完整执行至 `progress=100% processed=274/274`，全程无 `Error`。
