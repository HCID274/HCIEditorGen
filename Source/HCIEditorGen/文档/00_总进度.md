...
- 切片编号：`Step3-Slice3`
- 当前状态：`开发中`
- 切片目标：查询解析与混合召回（规则检索主路径 + 可选语义改写）。
- 真实查询场景（固定）：
  - `Q1`：帮我找森林关卡可复用、伤害不高、带控制倾向的技能。
  - `Q2`：找一个火系高爆发技能做 Boss 二阶段参考。
  - `Q3`：找和 `fire_01` 风格接近但伤害更低的候选。
- 验收动作（UE）：
  - 输入 `Q1/Q2/Q3` 查询可返回候选或可执行空结果建议；
  - 查询日志包含：解析后的过滤条件、候选数量、TopK 列表；
  - 无命中时给出下一步建议（而非静默失败）；
  - 全流程无 Error 级别日志。
- 通过后动作：进入 `Step3-Slice4`（排序与命中理由）。
...
- `2026-02-13`：用户确认回归修复 `Pass`，进入 `Step3-Slice2`。
- `2026-02-13`：完成 `Step3-Slice2` 代码实现：新增 `FHCIAbilityKitSearchIndexService`（全量构建/增量刷新/覆盖率统计），并接入 Editor 启动与 Import/Reimport 成功路径；命令行编译通过，待 UE 手测。
- `2026-02-13`：用户确认 `Step3-Slice2` UE 手测通过（`mode=incremental_refresh docs=1`，覆盖率字段完整）；进入 `Step3-Slice3`。
