...
static EHCIAgentPlanRiskLevel HCI_ToPlanRiskLevel(EHCIToolCapability Capability)
{
	switch (Capability)
	{
...
static bool HCI_StepFromTool(
	...
	const FHCIAgentPlanStep::FUiPresentation* UiPresentation,
	FHCIAgentPlanStep& OutStep,
	FString& OutError)
{
	const FHCIToolDescriptor* Tool = ToolRegistry.FindTool(FName(ToolName));
	if (!Tool)
	{
	...
	OutStep.bRequiresConfirm = (Tool->Capability != EHCIToolCapability::ReadOnly) || Tool->bDestructive;
	...
	FString& OutError)
{
	...
	if (!FHCIAgentPlanContract::ValidateMinimalContract(OutPlan, ContractError))
	{
...
static bool HCI_TryParseOptionalUiPresentation(
	...
	FHCIAgentPlanStep::FUiPresentation& OutUi,
	FString& OutError)
{
	OutUi = FHCIAgentPlanStep::FUiPresentation();
...
static TSharedPtr<FJsonValue> HCI_NormalizeJsonValueForArgSchema(
	const FHCIToolArgSchema& ArgSchema,
	const TSharedPtr<FJsonValue>& InValue)
{
	...
	if (ArgSchema.ValueType == EHCIToolArgValueType::StringArray && InValue->Type == EJson::String)
	{
		FString RawString;
...
static FString HCI_SerializeEnvContext(
	...
	const TArray<FHCIAgentPlannerEnvAssetEntry>& Assets)
{
	...
	{
		const FHCIAgentPlannerEnvAssetEntry& Entry = Assets[Index];
	...
	bool& bOutInjected)
{
	...
	TArray<FHCIAgentPlannerEnvAssetEntry> ScannedAssets;
	FString ScanError;
...
static FHCIAgentPromptBundleOptions HCI_MakePromptBundleOptions(const FHCIAgentPlannerBuildOptions& Options)
{
	FHCIAgentPromptBundleOptions BundleOptions;
...
{
	const FHCIToolDescriptor* Tool = ToolRegistry.FindTool(ToolName);
	if (Tool == nullptr)
	{
	...
	for (const FHCIToolArgSchema& ArgSchema : Tool->ArgsSchema)
	{
	...
	FString& OutError)
{
	...
	for (int32 StepIndex = 0; StepIndex < StepCount; ++StepIndex)
	{
		const FHCIAgentPlanStep& Step = InOutPlan.Steps[StepIndex];
	...
	TArray<FHCIAgentPlanStep> ReorderedSteps;
	...
	FString& OutError)
{
	...
	if (FHCIAgentPlanValidator::ValidatePlan(Plan, ToolRegistry, Validation))
	{
		return true;
	}
	...
	FString& OutError)
{
	...
	for (int32 Index = 0; Index < Steps->Num(); ++Index)
	{
		...
		FHCIAgentPlanStep::FUiPresentation UiPresentation;
		if (!HCI_TryParseOptionalUiPresentation(StepObject, Index, UiPresentation, OutError))
		{
			return false;
		}

		FHCIAgentPlanStep& Step = OutPlan.Steps.AddDefaulted_GetRef();
		if (!HCI_StepFromTool(
				ToolRegistry,
				*StepId,
				*ToolName,
...
static bool HCI_BuildKeywordPlan(
	...
	FString& OutError)
{
	FHCIKeywordPlannerProvider Provider;
	FHCIAgentPlannerBuildOptions Options;
	FHCIAgentPlannerResultMetadata Metadata;
	return Provider.BuildPlan(UserText, RequestId, ToolRegistry, Options, OutPlan, OutRouteReason, Metadata, OutError);
}
	...
	FString& OutError)
{
	...
	if (Options.LlmMockMode != EHCIAgentPlannerLlmMockMode::None &&
	...
	case EHCIAgentPlannerLlmMockMode::ContractInvalid:
		...
		return false;
	case EHCIAgentPlannerLlmMockMode::ValidButMisroutedScanAssets:
		OutPlan.RequestId = RequestId;
		OutPlan.Intent = TEXT("scan_assets");
		OutRouteReason = TEXT("fallback_scan_assets");
		{
			FHCIAgentPlanStep& ScanStep = OutPlan.Steps.AddDefaulted_GetRef();
			if (!HCI_StepFromTool(
					ToolRegistry,
		...
		return true;
	case EHCIAgentPlannerLlmMockMode::None:
	default:
		return HCI_BuildKeywordPlan(UserText, RequestId, ToolRegistry, OutPlan, OutRouteReason, OutError);
	}
}
...
using FHCIPlannerAsyncCallback = TFunction<void(bool, FHCIAgentPlan, FString, FHCIAgentPlannerResultMetadata, FString)>;

struct FHCIAsyncPlanBuildState : public TSharedFromThis<FHCIAsyncPlanBuildState, ESPMode::ThreadSafe>
{
	FString UserText;
	FString RequestId;
	const FHCIToolRegistry* ToolRegistry = nullptr;
	FHCIAgentPlannerBuildOptions Options;
...
static TArray<TSharedRef<FHCIAsyncPlanBuildState, ESPMode::ThreadSafe>> GHCIPlannerAsyncInFlightStates;

static void HCI_RegisterAsyncPlanBuildState(const TSharedRef<FHCIAsyncPlanBuildState, ESPMode::ThreadSafe>& State)
{
	GHCIPlannerAsyncInFlightStates.Add(State);
}

static void HCI_UnregisterAsyncPlanBuildState(const TSharedRef<FHCIAsyncPlanBuildState, ESPMode::ThreadSafe>& State)
{
	for (int32 Index = GHCIPlannerAsyncInFlightStates.Num() - 1; Index >= 0; --Index)
	{
		if (&GHCIPlannerAsyncInFlightStates[Index].Get() == &State.Get())
		{
			GHCIPlannerAsyncInFlightStates.RemoveAtSwap(Index);
			return;
		}
	}
}
...
static void HCI_CompleteAsyncPlanBuild(
	const TSharedRef<FHCIAsyncPlanBuildState, ESPMode::ThreadSafe>& State,
	const bool bBuilt,
	FHCIAgentPlan&& Plan,
	FString&& RouteReason,
	FHCIAgentPlannerResultMetadata&& Metadata,
...
static void HCI_FinishAsyncWithFailure(const TSharedRef<FHCIAsyncPlanBuildState, ESPMode::ThreadSafe>& State)
{
	FHCIAgentPlannerResultMetadata Metadata;
	...
	HCI_CompleteAsyncPlanBuild(State, false, FHCIAgentPlan(), FString(), MoveTemp(Metadata), MoveTemp(State->LastError));
}

static void HCI_StartAsyncRealHttpAttempt(const TSharedRef<FHCIAsyncPlanBuildState, ESPMode::ThreadSafe>& State)
{
			...
			ConfigError))
	{
		...
		HCI_FinishAsyncWithFailure(State);
		return;
	}
	...
	State->ActiveRequest = FHCIAgentLlmClient::CreateChatCompletionsHttpRequest(ProviderConfig, RequestBody);
	if (!State->ActiveRequest.IsValid())
	{
	...
	TWeakPtr<FHCIAsyncPlanBuildState, ESPMode::ThreadSafe> WeakState = State;
	State->ActiveRequest->OnProcessRequestComplete().BindLambda(
		...
		{
			const TSharedPtr<FHCIAsyncPlanBuildState, ESPMode::ThreadSafe> Pinned = WeakState.Pin();
			...
			if (!bSucceeded || !HttpResponse.IsValid() || HttpResponse->GetResponseCode() < 200 || HttpResponse->GetResponseCode() >= 300)
			{
				...
				HCI_FinishAsyncWithFailure(Pinned.ToSharedRef());
				return;
			}
				...
				FString LlmError;
				if (!FHCIAgentLlmClient::TryExtractMessageContentFromResponse(HttpResponse->GetContentAsString(), Content, LlmErrorCode, LlmError))
				{
...
const TCHAR* FHCILlmPlannerProvider::GetName() const
{
	return TEXT("llm");
}

bool FHCILlmPlannerProvider::IsNetworkBacked() const
{
	return true;
}

bool FHCILlmPlannerProvider::BuildPlan(
	const FString& UserText,
	const FString& RequestId,
	const FHCIToolRegistry& ToolRegistry,
	const FHCIAgentPlannerBuildOptions& Options,
	...
	FString& OutError)
{
	...
	OutMetadata.PlannerProvider = TEXT("llm");
	OutMetadata.ProviderMode = Options.bUseRealHttpProvider ? TEXT("real_http") : TEXT("mock");
	...
	{
		OutMetadata.FallbackReason = TEXT("llm_sync_real_http_deprecated");
	...
	int32 AttemptsUsed = 0;
	FString LastFallbackReason = TEXT("none");
	FString LastErrorCode = TEXT("-");
	FString LastError;
...
void FHCILlmPlannerProvider::BuildPlanAsync(
	const FString& UserText,
	const FString& RequestId,
	const FHCIToolRegistry& ToolRegistry,
	const FHCIAgentPlannerBuildOptions& Options,
	TFunction<void(bool, FHCIAgentPlan, FString, FHCIAgentPlannerResultMetadata, FString)>&& OnComplete)
{
	...
	TSharedRef<FHCIAsyncPlanBuildState, ESPMode::ThreadSafe> State =
		MakeShared<FHCIAsyncPlanBuildState, ESPMode::ThreadSafe>();
