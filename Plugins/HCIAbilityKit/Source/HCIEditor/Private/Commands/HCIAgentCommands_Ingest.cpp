...
DEFINE_LOG_CATEGORY_STATIC(LogHCIIngest, Log, All);

namespace
{
static bool HCI_TryLoadLatestManifest(FHCIIngestManifest& OutManifest, FString& OutError)
{
...
void HCI_RunAbilityKitIngestDumpLatestCommand(const TArray<FString>& Args)
{
	...
	if (!HCI_TryLoadLatestManifest(Manifest, Error))
	{
		UE_LOG(
			LogHCIIngest,
			Warning,
			TEXT("[HCI][Ingest] dump_latest failed reason=%s"),
	...
	UE_LOG(
		LogHCIIngest,
		Display,
		TEXT("[HCI][Ingest] latest batch_id=%s source_app=%s preflight_ok=%s files=%d target_root=%s manifest=%s"),
	...
	{
		UE_LOG(LogHCIIngest, Display, TEXT("[HCI][Ingest] warnings=%d first=%s"),
			Manifest.PreflightWarnings.Num(),
			*Manifest.PreflightWarnings[0]);
	}
	if (Manifest.PreflightErrors.Num() > 0)
	{
		UE_LOG(LogHCIIngest, Display, TEXT("[HCI][Ingest] errors=%d first=%s"),
...
void HCI_RunAbilityKitIngestImportLatestCommand(const TArray<FString>& Args)
{
	...
	if (!HCI_TryLoadLatestManifest(Manifest, Error))
	{
		UE_LOG(
			LogHCIIngest,
			Error,
			TEXT("[HCI][Ingest] import_latest failed reason=%s"),
			Error.IsEmpty() ? TEXT("-") : *Error);
		return;
	}
	...
	if (!Manifest.bPreflightOk)
	{
		...
		UE_LOG(
			LogHCIIngest,
			Error,
			TEXT("[HCI][Ingest] import_latest rejected reason=preflight_not_ok batch_id=%s first_error=%s"),
			Manifest.BatchId.IsEmpty() ? TEXT("-") : *Manifest.BatchId,
			*Err);
		return;
	}
	...
	if (!HCI_IsValidGameContentPath(Manifest.SuggestedUnrealTargetRoot))
	{
		UE_LOG(
			LogHCIIngest,
			Error,
			TEXT("[HCI][Ingest] import_latest rejected reason=invalid_target_root target_root=%s"),
			Manifest.SuggestedUnrealTargetRoot.IsEmpty() ? TEXT("-") : *Manifest.SuggestedUnrealTargetRoot);
		return;
	}
	...
	if (Choice != EAppReturnType::Yes)
	{
		UE_LOG(
			LogHCIIngest,
			Warning,
			TEXT("[HCI][Ingest] import_latest cancelled_by_user batch_id=%s"),
			Manifest.BatchId.IsEmpty() ? TEXT("-") : *Manifest.BatchId);
		return;
	}
	...
	for (const FHCIIngestManifestFile& File : Manifest.Files)
	{
		...
		if (!FPaths::FileExists(AbsFilePath))
		{
			UE_LOG(
				LogHCIIngest,
				Error,
				TEXT("[HCI][Ingest] import_latest failed reason=staged_file_missing rel=%s abs=%s"),
	...
	if (Tasks.Num() <= 0)
	{
		UE_LOG(LogHCIIngest, Error, TEXT("[HCI][Ingest] import_latest failed reason=no_tasks"));
		return;
	}
	...
	UE_LOG(
		LogHCIIngest,
		Display,
		TEXT("[HCI][Ingest] import_latest ok batch_id=%s tasks=%d imported_object_paths=%d sample=%s"),
		Manifest.BatchId.IsEmpty() ? TEXT("-") : *Manifest.BatchId,
		Tasks.Num(),
		TotalImported,
		*Sample);
...
void FHCIAgentDemoConsoleCommands::StartupIngestCommands()
{
	if (!IngestDumpLatestCommand.IsValid())
	{
		IngestDumpLatestCommand = MakeUnique<FAutoConsoleCommand>(
			TEXT("HCI.IngestDumpLatest"),
			TEXT("Stage N ingest: dump latest manifest summary. Usage: HCI.IngestDumpLatest"),
	...
	if (!IngestImportLatestCommand.IsValid())
	{
		IngestImportLatestCommand = MakeUnique<FAutoConsoleCommand>(
			TEXT("HCI.IngestImportLatest"),
			TEXT("Stage N ingest: import staged files from latest batch (requires confirmation). Usage: HCI.IngestImportLatest"),
			FConsoleCommandWithArgsDelegate::CreateStatic(&HCI_RunAbilityKitIngestImportLatestCommand));
	}
}

void FHCIAgentDemoConsoleCommands::ShutdownIngestCommands()
{
