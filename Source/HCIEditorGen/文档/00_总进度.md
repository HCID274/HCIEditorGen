...
- 切片编号：`Stage F - Slice F7`
- 当前状态：`F7 已通过（用户 UE 手测 Pass）`
- 切片目标：`ExecutorReview` 定位闭环：在 F6 桥接生成的审阅 `DryRunDiffReport` 上提供按行定位命令，支持 `actor -> camera_focus` 与 `asset -> sync_browser`，保持 `simulate_dry_run` 不触发真实资产写入。
- 通过后动作：已切换到 `Stage F - Slice F8`（待定义/待冻结），继续 F 段联调收尾。
